...
Перед тобой класс `OnlineSalesRegisterCollector`.
Он отвечает за работу онлайн-кассы.
...
Класс содержит:
  ...
  - словарь `tax_rate`, где записана налоговая ставка на товары. 
    Она составляет 10% или 20% от стоимости.
...
Замечание. Твоя задача - добавить в класс методы. Из них - девять обязательных и один дополнительный 

А теперь - по порядку о каждом методе 

#### 1. Напиши геттеры

В чеке печатают названия товаров и их количество. Но атрибуты `name_items` и 
`number_items` - приватные. К ним нельзя обратиться напрямую. 
...
В теле метода напиши условия: 
  - Если в названии товара 0 или больше 40 символов, выводится исключение 
    `ValueError`. Оно печатает сообщение: `Нельзя добавить товар, если в его названии нет символа или их больше 40`
  - Если названия товара нет в списке `item_price`, выводится исключение 
    `Nameerror` с текстом `Позиция отсутствует в товарнои справочнике`. 
...
Логика такая: метод содержит пустой список `total` и добавляет в него цены товаров из списка `name_items`. Их он берет из словаря 
`item_price`. 

Еще в методе есть условие: 
...
Напиши метод `ten_percent_tax_calculation`. Он рассчитывает НДС товаров у которых ставка 10%ю 
...
Напиши статистический метод `get_telephone_number`. Он возвращает номер телефона покупателя. 

На вход метод принимает аргумент `telephone_number` Это лесять цифр после `+7`. 
...
Чтобы метод вернул корректный номер, используй в теле условия: 
  ...
  - Если в аргумеенте больше 10 символов, выводится исключение `ValueError` с текстом `Необходимо ввести 10 цифр после "+7"`; 
  - В остальных случаях метод возвращает полный номер телефона. 
  - Подсказки
    - Чтобы принудительно вызвать исключение, испольщуй оператор `raise`. 
    - Чтобы исключение вывело конкретное сообщение, передай текст
    после имени исключения, в круглых скобках. Например, 
    `ValueError('Недопустимое значение')`. 
    - Добавить элемент в список можно через `append()`. 
    - Чтобы удалить элемент из списк, используй `remove()`.
    - Посчитать сумму жлементоа списка помодет функция `sum()`. 
...
Замечание. Это задание сложнее остальных. выполнять его не обязательно : ревьюер пример проект и без него. 
...
В `date` сохрани пять списков. По два значения в каждом: 
  ...
  #### Как будут оценивать твою работу 
    - В классе есть обязательные методы
    - Код запускается без ошибок 
    - Файл с заданием и переменные названы по правилам. 
    - В коде нет ничего лишнего - например, переменных, которые никак не используюся. 
